...
export default function Report({ id }: ReportProps) {
  ...
  const getUrgencyColor = (level: string) => {
    switch (level) {
      case "none": return { bg: "bg-emerald-500/10", text: "text-emerald-600 dark:text-emerald-400", border: "border-emerald-500/20", badge: "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" };
      case "routine": return { bg: "bg-amber-500/10", text: "text-amber-600 dark:text-amber-400", border: "border-amber-500/20", badge: "bg-amber-500/10 text-amber-600 dark:text-amber-400" };
      case "urgent": return { bg: "bg-orange-500/10", text: "text-orange-600 dark:text-orange-400", border: "border-orange-500/20", badge: "bg-orange-500/10 text-orange-600 dark:text-orange-400" };
      case "emergency": return { bg: "bg-red-500/10", text: "text-red-600 dark:text-red-400", border: "border-red-500/20", badge: "bg-red-500/10 text-red-600 dark:text-red-400" };
      default: return { bg: "bg-muted/50", text: "text-muted-foreground", border: "border-border", badge: "bg-muted text-muted-foreground" };
    }
  };
  ...
  const getRiskBand = (score: number) => {
    if (score < 0.25) return { label: "Low", color: "text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 border-emerald-500/20" };
    if (score < 0.5) return { label: "Moderate", color: "text-amber-600 dark:text-amber-400 bg-amber-500/10 border-amber-500/20" };
    if (score < 0.75) return { label: "High", color: "text-orange-600 dark:text-orange-400 bg-orange-500/10 border-orange-500/20" };
    return { label: "Critical", color: "text-red-600 dark:text-red-400 bg-red-500/10 border-red-500/20" };
  };
  ...
  return (
    <AppLayout>
      ...
      <div className="max-w-4xl mx-auto px-4 pb-16 space-y-8 print:px-0 print:pb-4 print:space-y-6">
        ...
        {scan.thresholdsCrossed && scan.thresholdsCrossed.length > 0 && (
          <section>
            ...
            <div className="bg-rose-500/10 border border-rose-500/20 rounded-xl p-5">
              <p className="text-xs font-bold text-rose-700 dark:text-rose-400 uppercase tracking-wide mb-3">
              ...
              <ul className="space-y-2">
                {scan.thresholdsCrossed.map((threshold, i) => (
                  <li key={i} className="flex items-center gap-2 text-sm font-semibold text-rose-800 dark:text-rose-300">
        ...
        {/* ─── FOOTER DISCLAIMER ─── */}
        <section>
          <div className="flex gap-4 p-5 bg-amber-500/10 border border-amber-500/30 rounded-xl text-amber-800 dark:text-amber-300 text-xs leading-relaxed">
            <ShieldAlert className="h-5 w-5 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
            <div>
              <p className="font-bold text-sm text-amber-900 dark:text-amber-200 mb-1">IMPORTANT CLINICAL DISCLAIMER</p>
              <p>
